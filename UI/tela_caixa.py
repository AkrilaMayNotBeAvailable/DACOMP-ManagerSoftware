...
class TelaCaixa(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.after(100, lambda: db.carregar_dados_vendas_do_dia(self.lista_produtos))
    ...
    def widgets_base_commands(self):
        ...
        campos["visualizar_dia"] = ctk.CTkButton(campos['formulario'], text="Visualizar Estatísticas do Dia", command=self.master.abrir_tela_estatisticas)
        ...
        campos["info_salvo"].grid(row=0, column=4, padx=5, pady=(10, 0), sticky="we")
    ...
    def widgets_available_products(self):
        ...
        for item in produtos:
            if item[1] != 0:
                filtro.append(item)
        dados = [item for item in dados if item[0] in [i[0] for i in filtro]]
